...
class InvalidTransactionError extends Error {
    constructor(m) {
        super(m)
        this.name = "Invalid Transaction Error"
    }
}
...
class IndividualAccount extends BankAccount{
    ...
    transferFunds(targetAccount, amount, actor) {
        ...
        targetAccount.transactions.push(new Transaction(targetAccount.accountNumber, amount, "deposit"))
    }
    getBalance(){
        return this.balance
    }
...
class Transaction{
    ...
    set transactionType(value){
        if(value.length === 0){
            throw new InsufficientFundsError("empty")
        }
